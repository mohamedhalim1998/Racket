...
(define (eval-under-env e env)
  (cond [(var? e) 
        ...
        [(mlet? e) (let ([v (eval-under-env (mlet-e e) env)])
                     (eval-under-env (mlet-body e) (cons (cons (mlet-var e) v) (var env))))]
                         ...
                         (let ([f  (closure-fun v1)]
                           ...
                           (if (fun-nameopt f)
                               (eval-under-env (fun-body f) (cons  (cons (fun-formal f) v2)
                                                                   (cons (cons (fun-nameopt f) (eval-under-env f en)) en)))
                               (eval-under-env (fun-body f)  (cons (cons (fun-formal f) v2) en))))
                                   (error  (format "not a function: ~v" e))))]
...
(define mupl-map (fun #f "_f"
                      (fun "_x" "lst"
                           (let([f  (var "lst")])
                           (ifaunit f
                                    (aunit)
                                    (apair (call (var "_f") (fst f)) (call (var "_x") (snd f))))))))
...
(define (compute-free-vars e)
  (fun-challenge (fun-nameopt e) (fun-formal e) (fun-body e) (set )))
