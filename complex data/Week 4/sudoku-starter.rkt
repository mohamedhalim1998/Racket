...
;; (define (solve-puzzle bd) false)
(check-expect (solve-puzzle BD4) BD4s)
(check-expect (solve-puzzle BD5) BD5s)
(check-expect (solve-puzzle BD7) false)

(define (solve-puzzle bd)
...
; Board -> Boolean
;; produce true if the board solved(if full)
;; ASSUME vaild board given
...
;; Board -> (listOf Board)
;; produce a list of boards and keep the valid ones
;; find an empty cell fill it with natrual[1:9]
(check-expect (next-boards BD1) (list (fill-square BD1 (r-c->pos 0 0) 1)
...
(check-expect (next-boards BD4) (list (fill-square BD4 (r-c->pos 0 3) 6)
                                      (fill-square BD4 (r-c->pos 0 3) 8)))
;; (define (next-boards bd) empty)
(define (next-boards bd)
  (valid-boards (genrate-boards bd (find-blank bd)) (find-blank bd)))

;; Board -> Position
;; produce the position of the first blank element at bd
...
(check-expect (genrate-boards BD1 0) (list (fill-square BD1 (r-c->pos 0 0) 1)
                                           (fill-square BD1 (r-c->pos 0 0) 2)
                                           (fill-square BD1 (r-c->pos 0 0) 3)
                                           (fill-square BD1 (r-c->pos 0 0) 4)
                                           (fill-square BD1 (r-c->pos 0 0) 5)
...
(check-expect (genrate-boards BD4 3) (list (fill-square BD4 (r-c->pos 0 3) 1)
                                           (fill-square BD4 (r-c->pos 0 3) 2)
                                           (fill-square BD4 (r-c->pos 0 3) 3)
                                           (fill-square BD4 (r-c->pos 0 3) 4)
                                           (fill-square BD4 (r-c->pos 0 3) 5)
                                           (fill-square BD4 (r-c->pos 0 3) 6)
                                           (fill-square BD4 (r-c->pos 0 3) 7)
                                           (fill-square BD4 (r-c->pos 0 3) 8)
                                           (fill-square BD4 (r-c->pos 0 3) 9))) 
(define (genrate-boards bd p)
  (local [(define (fn e) (fill-square bd p (+ e 1)))]
    (build-list 9 fn)))

;; (listof Board) -> (listOf Board)
;; produce only the valid boards
;; (define (valid-boards lobd) empty)
(check-expect (valid-boards (list (fill-square BD1 (r-c->pos 0 0) 1)
                                  (fill-square BD1 (r-c->pos 0 0) 2)
                                  (fill-square BD1 (r-c->pos 0 0) 3)
                                  (fill-square BD1 (r-c->pos 0 0) 4)
                                  (fill-square BD1 (r-c->pos 0 0) 5)
                                  ...
                                  (fill-square BD1 (r-c->pos 0 0) 7)
                                  (fill-square BD1 (r-c->pos 0 0) 8)
                                  (fill-square BD1 (r-c->pos 0 0) 9)) 0)
...
(check-expect (valid-boards(list (fill-square BD4 (r-c->pos 0 3) 1)
                                 (fill-square BD4 (r-c->pos 0 3) 2)
                                 (fill-square BD4 (r-c->pos 0 3) 3)
                                 (fill-square BD4 (r-c->pos 0 3) 4)
                                 (fill-square BD4 (r-c->pos 0 3) 5)
                                 ...
                                 (fill-square BD4 (r-c->pos 0 3) 7)
                                 (fill-square BD4 (r-c->pos 0 3) 8)
                                 (fill-square BD4 (r-c->pos 0 3) 9)) 3)
              (list (fill-square BD4 (r-c->pos 0 3) 6)
                    (fill-square BD4 (r-c->pos 0 3) 8)))
;;(define (valid-boards lobd pos) empty)


(define (valid-boards lobd pos)
  (local [(define (fn bd)  (not (in-units? bd pos UNITS (read-square bd pos))))]
    (filter fn lobd)))

;; pos (listof pos)-> Boolean
;; pruduce true if the pos in given unit
(check-expect (get-unit? 2 (first ROWS)) true)
(check-expect (get-unit? 10 (first ROWS)) false)
;; (define (get-unit? n lop) false)

(define (get-unit? pos lop)
  (cond[(empty? lop) false]
       [else (or (= pos (first lop)) (get-unit? pos (rest lop)))]))


;; Board pos -> boolean
;; produce true if the number is in rhe given unit
;; (define (in-units? bd pos n) false)
(check-expect (in-units? (fill-square BD4 (r-c->pos 0 3) 3) 3 UNITS 3) true)
(check-expect (in-units? (fill-square BD4 (r-c->pos 0 3) 6) 3 UNITS 6) false)

(define (in-units? bd pos units n)
  (cond [(empty? units) false]
        [else (if (get-unit? pos (first units))
                  (or (in-unit? bd (first units) pos n) (in-units? bd pos (rest units) n))
                  (in-units? bd pos (rest units) n))]))

;; Board (listof pos) number -> Boolean
;; pruduce true if the n in given unit
(check-expect (in-unit? BD4 (first ROWS) 2 2) true)
(check-expect (in-unit? (fill-square BD4 (r-c->pos 0 3) 6) (first ROWS) 3 6) false)
(check-expect (in-unit? (fill-square BD4 (r-c->pos 0 3) 5) (first(rest(rest(rest COLS)))) 3 5) true)
(define (in-unit? bd lop pos n)
  (cond[(empty? lop) false]
       [(= pos (first lop)) (or false (in-unit?  bd (rest lop) pos n))]
       [else (if (number? (read-square bd (first lop)))
              (or (= n (read-square bd (first lop))) (in-unit?  bd (rest lop) pos n))
              (or false (in-unit?  bd (rest lop) pos n)))]))
