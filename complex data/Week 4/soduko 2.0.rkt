...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname |soduko 2.0|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(define (next-boards bd)
  (valid-boards (genrate-boards bd (find-blank bd))))
...
(define (valid-boards lobd)
  (local [(define (fn bd)  (not (in-units? bd  UNITS )))]
    (filter fn lobd)))
...
(check-expect (in-units? (fill-square BD4 (r-c->pos 0 3) 3) UNITS ) true)
(check-expect (in-units? (fill-square BD4 (r-c->pos 0 3) 6) UNITS ) false)

(define (in-units? bd units)
  (cond [(empty? units) false]
        [else(or (in-unit-lop? bd (first units) (first units)) (in-units? bd (rest units)))]))


(define (in-unit-lop? bd lop1 lop2)
  (cond[(empty? lop1) false]
       [else (or (in-unit? bd lop2 (first lop1)) (in-unit-lop? bd (rest lop1) lop2))]))
;; Board (listof pos) number -> Boolean
;; pruduce true if the n in given uni
;(check-expect (in-unit? BD4 (first ROWS) 2) true)
(check-expect (in-unit? (fill-square BD4 (r-c->pos 0 3) 6) (first ROWS) 3) false)
(check-expect (in-unit? (fill-square BD4 (r-c->pos 0 3) 5) (first(rest(rest(rest COLS)))) 3) true)
(define (in-unit? bd lop pos)
  (cond[(empty? lop) false]
       [(= pos (first lop)) (or false (in-unit?  bd (rest lop) pos))]
       [else (if (and (number? (read-square bd pos)) (number? (read-square bd (first lop))))
              (or (= (read-square bd pos) (read-square bd (first lop))) (in-unit?  bd (rest lop) pos))
              (or false (in-unit?  bd (rest lop) pos)))]))
